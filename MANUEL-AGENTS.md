...
7. **🎨 Générer l'image (maquette)** : prévisualisation IA — utilise un prompt visuel épuré (sans jargon caméra ni syntaxe Midjourney, contrairement au « Prompt 8K » ci-dessus qui lui est fait pour être copié dans Midjourney/un photographe humain). Tente d'abord **Gemini** (clé Google) puis, si indisponible (quota gratuit insuffisant sans facturation activée, ou hors-ligne), **repli automatique** sur Pollinations.ai (gratuit, sans clé). Dans les deux cas, c'est une **maquette/prévisualisation**, jamais une vraie photo du site.
...
- Bouton **GPS** : suit ta position en temps réel et indique le **site le plus proche** et la distance. Un **point bleu** (façon Google Maps) marque ta position exacte sur la carte, entouré d'un cercle indiquant la précision réelle du signal. Un indicateur de qualité s'affiche aussi : **🎯 précis** (≤ 20 m, vrai signal satellite), **📶 correct** (≤ 50 m), ou **📡 approximatif** (Wi-Fi/réseau, souvent 100 m ou plus — fréquent en intérieur, à l'identique à chaque test si on ne bouge pas). **Sortir à ciel ouvert** est le seul moyen d'obtenir un vrai signal GPS précis ; ce n'est pas un réglage de l'app.
- **Déplacer la carte à la main** (pour regarder ailleurs) arrête temporairement le recentrage automatique du GPS — un bouton **🎯 Recentrer** apparaît pour revenir sur ta position en un tap.
- **🧭 Navigation GPS** : choisis une destination dans la liste déroulante (ou clique **« 🧭 Naviguer ici »** dans la fiche d'un site) → **🧭 Naviguer**.
  ...
  - **Avec une clé OpenRouteService** *(gratuite, voir le champ dans la carte Navigation)* : la carte affiche un **vrai itinéraire qui suit les routes** (comme Google Maps), avec distance routière et durée estimée. Si aucune route n'est cartographiée à cet endroit (zones isolées), ou si l'itinéraire renvoyé est manifestement aberrant (ex. énorme détour dû à des données cartographiques mal connectées — zone portuaire, ferry), l'app **repasse automatiquement** au guidage à vol d'oiseau, sans planter.
